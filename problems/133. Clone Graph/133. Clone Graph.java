...
//                 THIS IS THE POINT WHY WE USED NODE[] INSTEAD OF BOOLEAN[] ARRAY
	...
	public Node cloneGraph(Node node) {
		...
		Node[] visited = new Node[101]; // in this question we will create an array of Node(not boolean) why ? , because
										// i have to add all the adjacent nodes of particular vertex, whether it's
										// visited or not, so in the Node[] initially null is stored, if that node is
										// visited, we will store the respective node at the index, and can retrieve
										// that easily.
